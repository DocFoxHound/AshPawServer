...
class World {
  public:
    ...
    [[nodiscard]] std::optional<TransitionLink> transition(std::string_view id) const;
    ...
    EntityId spawn_player(Vec2 position, std::int32_t z = 0);
    bool despawn(EntityId id);
    bool set_movement_intent(EntityId id, MovementIntent intent);
    bool set_entity_position(EntityId id, Vec2 position, std::int32_t z = 0);
    InteractionResult interact(EntityId entity_id, std::string_view target_id);
    void tick(float delta_seconds);

  private:
    [[nodiscard]] EntityId next_entity_id();
    [[nodiscard]] EntityState resolve_spawn_position(Vec2 requested_position, std::int32_t requested_z);
    [[nodiscard]] bool can_spawn_at(Vec2 position, std::int32_t z) const;
    [[nodiscard]] bool is_position_blocked(Vec2 position, std::int32_t z) const;
    [[nodiscard]] bool is_occupied_by_closed_object(Vec2 position, std::int32_t z) const;
    [[nodiscard]] bool is_occupied_by_entity(Vec2 position, std::int32_t z, EntityId ignore_entity_id = 0) const;
